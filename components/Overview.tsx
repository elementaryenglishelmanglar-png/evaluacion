...
export default function Overview() {
  const [stats, setStats] = useState(appStore.getStats());
  const [recent, setRecent] = useState(appStore.getRecentActivity());
  const [studentCounts, setStudentCounts] = useState(appStore.getStudentCountsByGrade());

  // Force refresh for demo purposes when component mounts
  useEffect(() => {
    const interval = setInterval(() => {
      setStats(appStore.getStats());
      setRecent(appStore.getRecentActivity());
      setStudentCounts(appStore.getStudentCountsByGrade());
    }, 2000);
