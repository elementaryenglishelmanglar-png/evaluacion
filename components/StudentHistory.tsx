...
import { QualitativeGrade, EvaluationRecord, ActionPlan, ActionStatus, StudentLog } from '../types';
import { X, Activity, ClipboardList, TrendingUp, AlertTriangle, Calendar, FileText, CheckCircle2, XCircle, Clock, User, Target, UserPlus, Edit2, ArrowUpCircle, UserMinus, ArrowRight } from 'lucide-react';
...
export default function StudentHistory({ studentId, onClose }: StudentHistoryProps) {
    ...
    const [clinicalData, setClinicalData] = React.useState<any>(null);
    ...
    React.useEffect(() => {
        const loadData = async () => {
            setLoading(true);
            try {
                const s = await appStore.getStudentById(studentId);
                const data = await appStore.getStudentClinicalData(studentId);
                setStudent(s);
    ...
    if (loading) return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-full max-w-4xl bg-white h-full shadow-2xl flex items-center justify-center">
                <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-indigo-600"></div>
            </div>
        </div>
    );

    if (!student || !clinicalData) return null;
                        ...
                        <div className="max-w-2xl mx-auto">
                            ...
                            <div className="relative border-l-2 border-slate-200 pl-8 space-y-8">
                                ...
                                {clinicalData.timeline.map((event, idx) => {
                                    ...
                                    if (isLog) {
                                        const log = data as StudentLog;
                                        let icon = <FileText className="w-5 h-5 text-slate-500" />;
                                        ...
                                        switch (log.actionType) {
                                            ...
                                            case 'PROMOCION':
                                                ...
                                                title = "Promoción de Grado";
                                                break;
                                            case 'RETIRO':
                                                icon = <UserMinus className="w-5 h-5 text-rose-500" />;
                                                colorClass = "bg-rose-50 text-rose-700 border-rose-100";
                                                title = "Retiro del Estudiante";
                                                break;
                                        }

                                        return (
                                            <div key={idx} className="relative group">
                                                <div className={`absolute -left-[41px] top-4 w-5 h-5 rounded-full border-4 border-white shadow-sm flex items-center justify-center bg-slate-400`}></div>

                                                <span className="text-xs font-bold text-slate-400 mb-2 block uppercase tracking-wider">
                                                    {new Date(event.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                                    <span className="ml-2 opacity-50 lowercase">{new Date(event.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                                                </span>

                                                <div className={`p-4 rounded-xl border shadow-sm ${colorClass} flex items-start gap-4`}>
                                                    <div className="p-2 bg-white rounded-lg border border-slate-200/50 shadow-sm shrink-0">
                                                        {icon}
                                                    </div>
                                                    <div>
                                                        <h4 className="font-bold text-sm mb-1">{title}</h4>
                                                        <p className="text-sm opacity-80 mb-2">{log.details ? JSON.stringify(log.details) : 'Sin detalles adicionales'}</p>

                                                        {log.actionType === 'PROMOCION' && log.details && (
                                                            <div className="flex items-center gap-2 text-xs font-bold bg-white/50 px-2 py-1 rounded inline-block">
                                                                <span>{log.details.from}</span>
                                                                <ArrowRight className="w-3 h-3" />
                                                                <span>{log.details.to}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
