...
export default function CompetencyLibrary() {
    ...
    useEffect(() => {
        const fetchIndicators = async () => {
            if (selectedCompId) {
                const inds = await appStore.getIndicators(selectedCompId);
                setIndicators(inds);
            } else {
                setIndicators([]);
            }
    ...
    const handleAddCompetency = async () => {
        ...
        await appStore.addCompetency(newComp);
    ...
    const handleDeleteCompetency = async (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (window.confirm('¿Eliminar competencia y sus indicadores?')) {
            await appStore.deleteCompetency(id);
    ...
    const handleAddIndicator = async () => {
        ...
        await appStore.addIndicator(newInd);
    ...
    const handleDeleteIndicator = async (id: string) => {
        await appStore.deleteIndicator(id);
        if (selectedCompId) {
            const inds = await appStore.getIndicators(selectedCompId);
            setIndicators(inds);
        }
                                            ...
                                            <div className="flex gap-2">
                                                {['Cognitive', 'Procedural', 'Attitudinal'].map(t => (
                                                    <button
                                                        ...
                                                        className={`text-[10px] font-bold px-2 py-1 rounded border transition-colors ${newIndType === t
                                ...
                                <div className="space-y-3">
                                    {indicators.map(ind => (
                                        <div key={ind.id} className="flex items-start gap-3 p-4 bg-white border border-slate-100 rounded-xl hover:shadow-sm group">
                                            <div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${ind.type === 'Cognitive' ? 'bg-blue-400' :
